...
##
# Custom Nextcloud Docker entrypoint script for working with the
# proof-of-concept for a version of Nextcloud that does not check the ownership
# of the `config.php` file.
#
...
capture_install_options() {
    ...
    ### Nextcloud issue #13277 ###
    if [ "${NEXTCLOUD_CHECK_CONFIG_OWNER:-true}" = "false" ]; then
        install_options=$install_options' --no-config-owner-check'
    fi
