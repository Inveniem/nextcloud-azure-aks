...
##
# @file
# This script publishes the Nginx reverse proxy and static content server to a
# Container Registry.
#
# Usage:
...
#
container_name="inveniem/nextcloud-nginx-middleware"
...
print_usage_and_exit() {
  ...
  {
    echo "Usage: ${0} <container registry> [docker image tag]"
    echo ""
  } >&2
...
"${CONTAINER_ENGINE}" build -t "${container_name_and_tag}" . ${build_args:-}
